...
def sendax(text, ip, values=False):
  a,p=axcall(text, 0)                                  #src call
  if (p>=len(text)) or (text[p]!=ord(">")): 
      print("src call not found")
      return
...
def axtostr(axbuf):
  ...
  le-=2
  c=udpcrc(b, le)
  if (b[le]!=chr(c & 0xff)) or (b[le+1]!=chr(c>>8)): return ""  #crc error
...
# wx.sendax(wx.beacontext("OE0AAA-1","CQ-1","",2,"/_",48.25,13.002,{"celsius":2,"winddir":36,"wind":83,"rain24h":37,"pressure":1013.76}," pywx test",False),("127.0.0.1",7001),{})
